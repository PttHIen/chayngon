...
if($_GET['getriderlist']=="ok")
{
	...
	?>	
		...
		<script>
            var listRider=[];
            var config = {
                apiKey: "AIzaSyC1Sb6hNrCbo4Oj6wqEP-nC5HURM8q9_mU",
                authDomain: "ngon365-cd75a.firebaseapp.com",
                databaseURL: "https://ngon365-cd75a.firebaseio.com",
                projectId: "ngon365-cd75a",
                storageBucket: "ngon365-cd75a.appspot.com",
                messagingSenderId: "242195038118"
            };
            firebase.initializeApp(config);
            var list = firebase.database().ref("tracking");
            ...
            function getData(data) {
                ...
                data.forEach(function (childRecord) {
                    var item_data = childRecord.val();
                    console.log("lat:"+item_data.rider_name,item_data.rider_lat);
                    console.log("long:"+item_data.rider_name,item_data.rider_long);
                    console.log("rider_name",item_data.rider_name);
                    console.log("rider_id",item_data.rider_id);
                    var rider_id=item_data.rider_id;
                    var keyRiderMap=`rider_${rider_id}`;
                    var mapRider=listRider[keyRiderMap];
                    if(typeof  mapRider !=="undefined"){
                        var latlng = new google.maps.LatLng(item_data.rider_lat,item_data.rider_long);
                        mapRider.setPosition(latlng);

                        console.log("mapRider",mapRider);
                        listRiderOnline[keyRiderMap]=mapRider;
                    }else{
                        var newRarker= new google.maps.Marker({
                            //position: new google.maps.LatLng(locations[i][1], locations[i][2]),
                            position: new google.maps.LatLng(item_data.rider_lat,item_data.rider_long),
                            map: map ,
                            title:'Người giao hàng',
                            icon: 'http://maps.google.com/mapfiles/ms/micons/motorcycling.png'
                        });
                       /* google.maps.event.addListener(newRarker, 'click', (function (newRarker, i) {
                            return function () {
                                infowindow.setContent(locations[i][0]);
                                infowindow.open(map, newRarker);
                            }
                        })(marker, i));*/
                        listRiderOnline[keyRiderMap]=newRarker;
                    }
                            ...
                            // Cretes the map
                             ...
                             function initialize() 
                             {
							    ...
							    var locations = [
								    ...
								    <?php  
								        ...
								        foreach($json_data['msg']['OnlineRiders'] as $str => $data) 
								        {
								            foreach($data['RiderLocation'] as $d=>$rd)
								            {
								                $var ='[a'.$rd['RiderLocation']['id'].'.b'.$rd['RiderLocation']['user_id'].', a'.$rd['RiderLocation']['id'].'.lat'.', a'.$rd['RiderLocation']['id'].'.long'.', a'.$rd['RiderLocation']['id'].'.countt'.',0,"rider_'.$rd['RiderLocation']['user_id'].'"],';
								                echo $var;
								            }
								        }
						                     ...
						                     foreach($json_data['msg']['OnlineRiders'] as $str => $data)
						                     {
						                         foreach($data['RiderLocation'] as $d=>$rd)
						                         {
						                             //$var ='center: new google.maps.LatLng'.'('. $rider_full_name= $rd['RiderLocation']['lat']." , ".$rd['RiderLocation']['long'].')';
						                              ...
						                              echo $var.','.'';
						                              //echo "icon: 'http://maps.google.com/mapfiles/ms/micons/motorcycling.png'";
			                    ...
			                    for (i = 0; i < locations.length; i++) 
			                    {
                                    ...
                                    listRider[locations[i][5]]=marker;
