...
class DuongApiController extends AppController
{
    ...
    public function showRestaurantsMongoDb1()
    {
        ...
        if ($this->request->isPost()) {
            ...
            foreach ($items as $item) {
                ...
                $itemRestaurant->{"0"}->distance = $distance;
    ...
    public function showRestaurantsMongoDb()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $m = new MongoClient();
            $db = $m->selectDB('live_ngon365');
            $collection = new MongoCollection($db, 'restaurant');
            $fields = [
                'id' => 1,
                'name' => 1,
                'image' => 1,
            ...
            $filter=[
                "list_menu"=> array('$exists' => true),
            ];
            $cursor=$collection->find([] ,$fields)->limit(10);
            $items = [];
...
//                die;
            ...
            if ($delivery == 1) {
                ...
                if ($this->Order->save($order_update)) {
                    ...
                    $order_detail[0]['User'] = $user_details[0]['User'];
