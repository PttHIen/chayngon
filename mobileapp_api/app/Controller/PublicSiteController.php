...
public function index(){
    ...
    public function verifyPhoneNo()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($verify == 0) {
                if ($this->PhoneNoVerification->save($phone_verify)) {


                    if (Lib::sendSmsVerification($phone_no, $code . ' ' . VERIFICATION_PHONENO_MESSAGE)) {
...
*/
    ...
    public function showRestaurantsFixData()
    {
        $this->loadModel("Restaurant");
        $this->loadModel("Order");
        $this->loadModel("RiderOrder");
        ...
        $restaurants = $this->Restaurant->getAllRestaurantsFixData();
        echo "<pre>";
        print_r($restaurants, false);
        echo "</pre>";
        die;
        foreach ($restaurants as $restaurant){
            $id=$restaurant['Restaurant']['id'];
            $vendor_data=$restaurant['Restaurant']['vendor_data'];
            $vendor_data=json_decode($vendor_data);

            $ImageUrl=$vendor_data->ImageUrl;
            $Category=$vendor_data->Category;
            $Cuisines=$vendor_data->Cuisines;
            $RestaurantName=$vendor_data->RestaurantName;
           //gui den table_address $RestaurantAddress=$vendor_data->RestaurantAddress;
            $ShortDescription=$vendor_data->ShortDescription;
            $Description=$vendor_data->Description;
            $TimeRange=$vendor_data->TimeRange; /* chuyen vao preparation time*/
            $PriceRange=$vendor_data->PriceRange;
            $PriceMin=$vendor_data->PriceRange; /* chuyen vao min_order_price*/



            $ResLat=$vendor_data->ResLat; /* chuyen vao preparation time*/
            $ResLng=$vendor_data->ResLng; /* chuyen vao restaurant_location*/
            $DistrictName =$vendor_data->DistrictName;/* chuyen vao restaurant_location*/
            $ProvinceName =$vendor_data->ProvinceName;/* chuyen vao restaurant_location*/
            $Address =$vendor_data->Address;/* chuyen vao restaurant_location*/
            $IsAvailableDate=$vendor_data->IsAvailableDate;
            $Busy=$vendor_data->Busy;
            $Keyword=$vendor_data->Keyword;




            $YoutubeUrl=$vendor_data->YoutubeUrl;
            $IsQualityMerchant=$vendor_data->IsQualityMerchant;
            $HasContract=$vendor_data->HasContract;
            $curent_restaurent['image']=$ImageUrl;
            $curent_restaurent['description']=description;
            $curent_restaurent['vendor_alias']="sdfsdf";
            $curent_restaurent['short_description']=$ShortDescription;
            $data = array(
                'id' => $id,
                'vendor_alias' => $ImageUrl,
            );
            echo "<pre>";
            print_r($data, false);
            echo "</pre>";
