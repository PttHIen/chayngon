   ...
   public function index(){
    ...
    public function showRestaurantOrders()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $restaurant_orders = $this->Restaurant->getAdminRestaurantOrders($page);

            $total = round($this->Restaurant->find('count')/20);

            $output['code'] = 200;


            $output['msg']['items'] = $restaurant_orders;
            $output['msg']['total'] = $total;
            echo json_encode($output);
    ...
    public function showAllUsers()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $users = $this->User->getAllAdminUsers($page);

            $total = round($this->User->find('count')/20);
            $output['code'] = 200;

            $output['msg']['items'] = $users;
            $output['msg']['total'] = $total;
            echo json_encode($output);
    ...
    public function showRestaurants()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if(isset($data['check']['page'])){
                $page = $data['check']['page'];
            }
            if(isset($data['check']['search'])){
                $search = $data['check']['search'];
            }
            if (empty($search)){
                $restaurants = $this->Restaurant->getAdminRestaurants($page);
                $total = ceil($this->Restaurant->find('count')/20);
            }
            else {
                $restaurants = $this->Restaurant->getAdminSearchRestaurants($page,$search);
                $total = ceil($this->Restaurant->getAdminTotalSearchRestaurants($search)/20);
            }
    ...
    public function get_list_vendor(){
        ...
        $list_vendor=$this->Vendor->loadVendor();
        echo json_encode($list_vendor);
    ...
    public function assignOrderToRider()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if($delivery == 0){
                ...
                $output['msg'] = "Bạn không thể chỉ định đơn đặt hàng này cho bất kỳ người lái nào vì người dùng sẽ tự lấy đồ ăn từ nhà hàng";
                echo json_encode($output);
                ...
                if ($this->RiderOrder->save($rider_order)) {
                    ...
                    $curl_date[$order_id] =
                        array (
                            ...
                            'order_status' => 'Đơn hàng đã được giao cho '.$rider_name,
                            'map_change' => "1",
                 ...
                 /*   PushNotification::sendPushNotification($device_token, "you have been assigned an order");
                    ...
                    $notification['notification']['title'] = "Đơn hàng đã được giao cho shipper";
                    $notification['notification']['body'] = 'Đơn hàng #'.$order_detail[0]['Order']['id'] .' '.$order_detail[0]['OrderMenuItem'][0]['name'];
                    $notification['notification']['badge'] = "1";
                    $notification['notification']['sound'] = "default";
                    $notification['notification']['icon'] = "";
                    $notification['notification']['type'] = "";
