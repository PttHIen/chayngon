...
// ==== Control Vars =======
        ...
        $ApiKey="F37B5ABDE07D5528E2D5E0F71C58CD";
        $SecretKey = "51FCB101CE3749566ADFFC969E5EFE";
        //--check number first------------------

        $strToNumber=str_replace('+84',"",$strToNumber);
        $msg =  urlencode( $msg );
        $base_url = "https://lookups.twilio.com/v1/PhoneNumbers/" . $strToNumber;
        $base_url = "http://rest.esms.vn/MainService.svc/json/SendMultipleMessage_V4_get?Phone=0$strToNumber&Content=$msg&ApiKey=$ApiKey&SecretKey=$SecretKey&SmsType=2&Brandname=Verify";
        $test_phone=false;
        if($test_phone) {
            ...
            $response='{"CodeResult":"100","CountRegenerate":0,"SMSID":"7c60f173-509e-4a82-8057-6efdfd59ecc238"}';
        }
        $response = json_decode($response);
        $response_decoded = json_decode(json_encode($response), True);
        if (array_key_exists('CodeResult', $response_decoded)) {
             if ($response_decoded['CodeResult'] == 100) {
