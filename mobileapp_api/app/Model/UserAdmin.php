...
class UserAdmin extends AppModel
{
    ...
    public function verifyPassword($email,$old_password){
        ...
        if ($passwordHash == $userData['UserAdmin']['password'] && $salt == $userData['UserAdmin']['salt']) {
    ...
    public function verify($email,$user_password)
    {
        ...
        if ($passwordHash == $userData[0]['UserAdmin']['password'] && $salt == $userData[0]['UserAdmin']['salt']) {
    ...
    public function loginAllUsers($email,$user_password)
    {
        ...
        if ($passwordHash == $userData[0]['UserAdmin']['password'] && $salt == $userData[0]['UserAdmin']['salt'] && $userData[0]['UserAdmin']['active'] == 1) {
    ...
    public function loginRestaurantAndRiderAndUser($email,$user_password)
    {
        ...
        if ($passwordHash == $userData[0]['User']['password'] && $salt == $userData[0]['User']['salt']) {
